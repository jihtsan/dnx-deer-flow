...
**Knowledge Base / LightRAG**: `knowledge_base` is operator-owned configuration for one LightRAG service and one workspace selected when that service starts. `app/knowledge/lightrag.py` is the strict data-plane adapter for health, upload, tracking, structured `/query/data`, and delete responses. It returns normalized fields only, never forwards raw health data, and never sends `LIGHTRAG-WORKSPACE`. `GET /api/features` converts health and pinned-version checks into `unconfigured`, `disabled`, `offline`, `incompatible`, or `ready` without exposing the endpoint or API key. Real contract coverage lives in `tests/integration/test_lightrag_contract.py` and requires `LIGHTRAG_CHECKOUT` to reference the pinned checkout.
