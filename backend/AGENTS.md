...
**Knowledge Base / LightRAG**: `knowledge_base` is operator-owned configuration for one LightRAG service and one workspace selected when that service starts. `app/knowledge/lightrag.py` is the strict data-plane adapter for health, upload, tracking, structured `/query/data`, and delete responses. It returns normalized fields only, never forwards raw health data, and never sends `LIGHTRAG-WORKSPACE`. `GET /api/features` converts health and pinned-version checks into `unconfigured`, `disabled`, `offline`, `incompatible`, or `ready` without exposing the endpoint or API key. `GET/POST/PATCH /api/knowledge/scope` manages one owner-visible Knowledge Scope and embeds that same redacted state. `GET/POST /api/knowledge/documents` lists persisted state and accepts a multipart `file` plus `Idempotency-Key`; the request stages and atomically renames the original file, then commits the document and ingestion job together before returning `202`. `app/knowledge/ingestion.py` owns process-local upload/tracking tasks, maps LightRAG states into `pending`, `indexing`, `ready`, or `failed`, and stores only stable sanitized failure reasons. Job leases, restart recovery, retry policy, and manual retry remain outside this boundary. `app/knowledge/storage.py` enforces NFKC filenames, server path isolation, exact extension/MIME pairs, 25 MiB per-file and 1 GiB singleton-Scope quotas, temp cleanup, and database-failure compensation; every filesystem operation is offloaded through `run_file_io`. The Harness repositories persist the singleton Scope plus documents/jobs; a checked and unique `singleton_key=1` enforces the global one-row invariant, and only owner-visible `ready` documents in an enabled Scope are retrieval candidates. Real LightRAG contract coverage lives in `tests/integration/test_lightrag_contract.py` and requires `LIGHTRAG_CHECKOUT` to reference the pinned checkout.
...
| **Knowledge Documents** (`/api/knowledge/documents`) | `GET /` - list owner-visible persisted documents for refresh recovery; `POST /` - accept one safe multipart file with `Idempotency-Key`, return `202` with local job tracking immediately, and schedule process-local LightRAG upload/status tracking |
