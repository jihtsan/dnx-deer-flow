...
**Nexus model catalog apply**: `scripts/nexus-apply-models.py` is the only fixed SSH entry point for Nexus-managed model delivery. It accepts schema version 1 JSON on stdin, validates the full candidate with `AppConfig`, replaces only the top-level `models` list through an atomic rename, preserves file permissions, and emits no credentials. Keep this surface narrow; it is not a general-purpose remote configuration API.

**Config Hot-Reload Boundary**: Gateway dependencies route through `get_app_config()` on every request, so per-run fields like `models[*].max_tokens`, `summarization.*`, `title.*`, `memory.*`, `subagents.*`, `tools[*]`, and the agent system prompt pick up `config.yaml` edits on the next message. `AppConfig` is intentionally **not** cached on `app.state` — `lifespan()` keeps a local `startup_config` variable for one-shot bootstrap work and passes it to `langgraph_runtime(app, startup_config)`.

**Knowledge Base / LightRAG**: `knowledge_base` is operator-owned configuration for one LightRAG service and one workspace selected when that service starts. `app/knowledge/lightrag.py` is the strict data-plane adapter for health, upload, tracking, popular graph labels, connected graphs, structured `/query/data`, and delete responses. It returns normalized fields only, reduces upstream paths to display filenames, never forwards raw health or arbitrary payload fields, and never sends `LIGHTRAG-WORKSPACE`. `GET /api/features` converts health and pinned-version checks into `unconfigured`, `disabled`, `offline`, `incompatible`, or `ready` without exposing the endpoint or API key. `GET/POST/PATCH /api/knowledge/scope` manages one owner-visible Knowledge Scope and embeds that same redacted state. The read-only `GET /api/knowledge/graph/labels`, `GET /api/knowledge/graph`, and `POST /api/knowledge/retrieval` routes require `knowledge:read`, an enabled owner-visible Scope, and a ready pinned data plane; the public retrieval modes exclude LightRAG's `bypass` mode and all query/node/token limits are bounded at the Gateway. `GET/POST /api/knowledge/documents` lists persisted state and accepts a multipart `file` plus `Idempotency-Key`; the request stages and atomically renames the original file, then commits the document and ingestion job together before returning `202`. `POST /api/knowledge/documents/{document_id}/retry` uses its own `Idempotency-Key` to reactivate an eligible `dead` job without creating a second job; every accepted key is retained in `knowledge_ingestion_retry_requests`, so an older replay remains inert after later retry rounds. `DELETE /api/knowledge/documents/{document_id}` requires `knowledge:write`, rejects non-terminal ingestion, requires the pinned LightRAG data plane to be ready for remote state, resolves managed LightRAG rows by the stable server filename and matching tracking ID, polls until the remote ID is confirmed absent, and preserves local state on upstream failure; successful local deletion removes the document, its cascaded job/retry ledger, and the managed original file. `app/knowledge/ingestion.py` is the single durable worker boundary: it discovers persisted work at Gateway startup, claims each job with a SQL `UPDATE ... RETURNING`, runs a bounded number of claims concurrently, caps non-terminal tracking polls per attempt, resolves the hot-reloaded LightRAG client before each attempt without claiming while unavailable, fences renew/progress/failure/completion by owner + attempt + unexpired lease, applies bounded exponential backoff, and recovers expired leases after crashes. Its internal states are `pending`, `leased`, `retry_wait`, `succeeded`, `dead`, and `cancelled`; API document states remain the smaller `pending`, `indexing`, `ready`, and `failed` model. A stable server-generated LightRAG filename plus exact paginated lookup reconciles upload-success/local-commit crashes, but LightRAG has no upload idempotency-key contract and this is not remote exactly-once. `app/knowledge/storage.py` enforces NFKC filenames, server path isolation, exact extension/MIME pairs, 25 MiB per-file and 1 GiB singleton-Scope quotas, temp cleanup, and database-failure compensation; every filesystem operation is offloaded through `run_file_io`. SQL access is async. The Harness repositories persist the singleton Scope plus documents/jobs; a checked and unique `singleton_key=1` enforces the global one-row invariant, and only owner-visible `ready` documents in an enabled Scope are retrieval candidates. Real LightRAG contract coverage lives in `tests/integration/test_lightrag_contract.py` and requires `LIGHTRAG_CHECKOUT` to reference the pinned checkout. The detailed transition and recovery contract is in `../docs/knowledge-ingestion-reliability.md`.

Infrastructure fields are **restart-required**. The authoritative list lives in `packages/harness/deerflow/config/reload_boundary.py::STARTUP_ONLY_FIELDS` and is mirrored by the standardised `"startup-only:"` prefix on the corresponding `Field(description=...)` in `AppConfig`, so IDE hover on those fields surfaces the reason inline (no need to context-switch into this table). Currently registered: `plugins`, `database`, `checkpointer`, `run_events`, `agent_storage`, `stream_bridge`, `sandbox`, `log_level`, `logging`, `channels`, `channel_connections`, `scheduler`, `mcp_tasks`, `run_ownership`. Adding a new restart-required field requires updating the registry; drift is pinned by `tests/test_reload_boundary.py`.

Knowledge document progress stores only the allowlisted LightRAG stage, optional
non-negative chunk count, and worker-stamped stage-change time. It intentionally
does not derive per-document percentages from workspace-global pipeline status.
...
| **Knowledge Data** (`/api/knowledge`) | `GET /graph/labels` - bounded popular labels; `GET /graph/search` - fuzzy entity-label search; `GET /graph/global` - merge disconnected components from all labels into a deduplicated global graph with explicit component/request/node truncation, a 5,000-node ceiling, and a 30-second request-level deadline that returns collected components as truncated; `GET /graph` - normalized connected graph compatibility route; `POST /retrieval` - normalized structured retrieval data for the five supported user modes, with Gateway-owned default limits. Short LightRAG operations use `lightrag.timeout_seconds`; retrieval uses `lightrag.query_timeout_seconds` because keyword extraction may invoke an LLM. |
| **Knowledge Documents** (`/api/knowledge/documents`) | `GET /` - list owner-visible documents plus sanitized ingestion diagnostics for refresh recovery; `POST /` - accept one safe multipart file with `Idempotency-Key`, return `202`, and wake the durable leased worker; `POST /{document_id}/retry` - idempotently reactivate an eligible dead job and return its current persisted state; `DELETE /{document_id}` - remove the matching LightRAG document and then the owner-visible local document, ingestion history, retry ledger, and managed original file |
...
- **Installation**: `POST /api/skills/install` extracts .skill ZIP archives to the current user's custom directory. The separate Nexus receiver adapter reuses the same guarded/atomic `UserScopedSkillStorage` path for explicitly selected `USER` targets, adds package digest/manifest/idempotency checks and a minimal redacted identity sidecar, and proves success only from enabled+loaded exact Observed state. Receiver phases persist in SQL across restart; production does not construct the adapter until the remaining service-auth, directory, trust, Secret, and SSH release gates are approved.
...
- `migrations/versions/0012_merge_knowledge_and_mcp_tasks.py` — no-op merge head joining the Knowledge catalog branch at `0009_repair_knowledge_retry_requests` with the upstream runtime branch at `0011_mcp_tasks`; keep both histories intact when adding later revisions
...
provider semantics plus the shared `http_v1`/`ssh_v1` binding metadata. The SSH
