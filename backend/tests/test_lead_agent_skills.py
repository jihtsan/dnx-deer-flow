...
def test_make_lead_agent_empty_skills_passed_correctly(monkeypatch):
    ...
    monkeypatch.setattr(lead_agent_module, "_load_enabled_available_skills", lambda available_skills, *, app_config, user_id=None, catalog_revision="0": [])
...
def test_make_lead_agent_custom_skill_allowlist_does_not_activate_tool_policy(monkeypatch):
    ...
    monkeypatch.setattr(lead_agent_module, "_load_enabled_available_skills", lambda available_skills, *, app_config, user_id=None, catalog_revision="0": [_make_skill("restricted", ["read_file", "web_search"]), _make_skill("legacy", None)])
...
def test_make_lead_agent_all_legacy_skills_preserve_all_tools(monkeypatch):
    ...
    monkeypatch.setattr(lead_agent_module, "create_agent", lambda **kwargs: kwargs)
    monkeypatch.setattr(lead_agent_module, "load_agent_config", lambda x, *, user_id=None: AgentConfig(name="test", skills=None))
    monkeypatch.setattr(lead_agent_module, "_load_enabled_available_skills", lambda available_skills, *, app_config, user_id=None, catalog_revision="0": [_make_skill("legacy", None)])
...
def test_default_lead_agent_does_not_apply_installed_skill_allowlists(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda available_skills, *, app_config, user_id=None, catalog_revision="0": [_make_skill("skill-reviewer", ["review_skill_package"])],
    )
    monkeypatch.setattr(
        "deerflow.tools.get_available_tools",
